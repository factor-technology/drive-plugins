...
**Claude Code**
    ...
    /plugin marketplace add factor-technology/drive-plugins
    /plugin install geosteering-agent@factor-drive
...
- **Claude Code** — `/plugin`, select `factor-drive`, toggle
  auto-update on.
- **Either host, declaratively** — in `~/.claude/settings.json`:

      {
        "extraKnownMarketplaces": {
          "factor-drive": {
            "source": {"source": "github", "repo": "factor-technology/drive-plugins"},
            "autoUpdate": true
          }
        }
      }
